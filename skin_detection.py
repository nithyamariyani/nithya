...
def cluster_matrix(dframe, dframe_removed, skin_cluster_label, height, width):
    # Ensure 'cluster' column is added to dframe_removed
    dframe_removed['cluster'] = -1

    # Concatenate the two DataFrames
    dframe = pd.concat([dframe, dframe_removed], ignore_index=False).sort_index()

    # Update 'cluster' column based on skin_cluster_label
    dframe['cluster'] = (dframe['cluster'] == skin_cluster_label).astype(int) * 255

    # Reshape to the required matrix form
    cluster_label_mat = np.asarray(dframe['cluster'].values).reshape(height, width).astype(np.uint8)
